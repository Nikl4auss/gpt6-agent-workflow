...
- Definitions: `.opencode/agents/gpt6-{sol,luna,astra}-trial.md` pin `openai/gpt-6-{sol,luna,astra}`; Sol/Astra deny edits, Luna allows edits. `opencode agent list` showed all three after additive global installation. Existing `opencode.jsonc` still has `openai/gpt-5.6-sol` and `sol-orchestrator`; existing GPT5 agent files were not changed.
